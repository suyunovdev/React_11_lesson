...
const CartPage = () => {
  ...
  return (
    <main className="overflow-x-hidden">
      <Navbar products={products?.products?.length} />
      <Portrait page="Cart" />
        ...
        <div className="px-5 md:px-20 flex flex-col md:flex-row items-start justify-center gap-10 my-5">
              ...
              {products?.products?.map((product, index) => (
                <tr key={index}>
                  ...
                  <td className="text-center">
                    <button className="cursor-pointer" onClick={() => deleteItem(product?.id)}>
          ...
          <div className="w-full flex flex-col flex-1 px-10 py-10 gap-10 bg-[#F9F1E7] items-center shadow-xl">
            ...
            <div className="w-full flex flex-row items-center justify-between">
              ...
              <span className="font-medium text-[#9F9F9F] text-xl">${products.subtotal}</span>
            </div>
            <div className="w-full flex flex-row items-center justify-between">
              <span className="font-semibold text-xl">Total</span>
              <span className="font-medium text-[#B88E2F] text-xl">${products.total}</span>
            </div>
            <Link to="/checkout">
